...
float bh1750_read(float *arrLuxVal, uint16_t *arrLuxRaw)
{
  uint8_t buf[32];
  ...
  if(ret != ESP_OK) {
    ESP_LOGW(TAG_BH1750, "BH1750 Reset-1");
    bh1750_reset();
    return -1;
  }
  ...
  if(ret != ESP_OK) {
    ESP_LOGW(TAG_BH1750, "BH1750 Reset-2");
    bh1750_reset();
    return 0;
  }
  uint16_t luxRaw = (uint16_t)(((uint16_t)(buf[0]<<8))|((uint16_t)buf[1]));
  luxVal = (float)luxRaw/1.2/resdiv;
  //ESP_LOGI(TAG_BH1750, "sensraw=%u | lux=%f | resdiv=%d", luxRaw, luxVal, resdiv);
...
void bh1750_init(void)
{
  ...
  ESP_LOGI(TAG_BH1750, "sda_io_num %d", PIN_SDA);
  conf.sda_io_num = PIN_SDA;
  conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
  ESP_LOGI(TAG_BH1750, "scl_io_num %d", PIN_SCL);
  conf.scl_io_num = PIN_SCL;
  conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
  ESP_LOGI(TAG_BH1750, "clk_speed %d", I2C_MASTER_FREQ_HZ);
  conf.master.clk_speed = I2C_MASTER_FREQ_HZ;
  ESP_LOGI(TAG_BH1750, "i2c_param_config %d", conf.mode);
  ESP_ERROR_CHECK(i2c_param_config(I2C_MASTER_NUM, &conf));
  ESP_LOGI(TAG_BH1750, "i2c_driver_install %d", I2C_MASTER_NUM);
  ESP_ERROR_CHECK(i2c_driver_install(I2C_MASTER_NUM, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0));
}

void bh1750_deinit(void)
{
  ESP_LOGI(TAG_BH1750, "i2c_driver_delete");
...
int bh1750_I2C_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint8_t cnt)
{
  ...
  ESP_LOGD(TAG_BH1750, "bh1750_I2C_write I2CAddress 0x%02X len %d reg 0x%02X", dev_addr,cnt,reg_addr);
  ...
  if(ret != ESP_OK) {
    ESP_LOGE(TAG_BH1750, "bh1750_I2C_write write data fail I2CAddress 0x%02X len %d reg 0x%02X", dev_addr,cnt,reg_addr);
  }
...
int bh1750_I2C_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *reg_data, uint8_t cnt)
{
  ...
  ESP_LOGD(TAG_BH1750, "bh1750_I2C_read I2CAddress 0x%02X len %d reg 0x%02X", dev_addr,cnt,reg_addr);
  ...
  if(reg_addr!=0xFF) {
    ...
    i2c_cmd_link_delete(cmd);
    if(ret != ESP_OK) {
      ESP_LOGE(TAG_BH1750, "bh1750_I2C_read write reg fail %d",ret);
      return ret;
    }
  }
  ...
  i2c_cmd_link_delete(cmd);
  if(ret != ESP_OK) {
    ESP_LOGE(TAG_BH1750, "bh1750_I2C_read read data fail %d",ret);
    return ret;
  }
